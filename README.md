...
- `license/`: コミュニティー配布向けのメール許可制テンプレート
- `webapp/`: 中央管理型Web App版の雛形
...
| `ADDITIONAL_SHIPPING_RATE` | 同一商品2個目以降の追加送料の割合。`0.75`（送料の75%）がデフォルト。シートのメニュー「設定: 追加送料の割合(%)を変更」からも変更可能。Optional |
...
配布先をメールアドレスで制限したい場合は、`license/` フォルダのテンプレートを使います。

ただし、Apps Scriptのコードを丸ごと渡す方式では、受け取った人がコードを編集できます。
そのため、メールチェックは実用上の制限にはなりますが、完全なコピー防止ではありません。
