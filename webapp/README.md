...
eBay DeveloperのClient ID / Client Secret / RuNameは、管理者側のスクリプトプロパティにだけ保存します。
利用者にはeBay Developer情報を入力させません。
...
4. スクリプトプロパティに `WEBAPP_SESSION_SECRET`、`EBAY_CLIENT_ID`、`EBAY_CLIENT_SECRET`、`EBAY_RUNAME` を設定します。
...
また、利用者からeBay DeveloperのClient Secretを預からないため、セキュリティー面でも扱いやすくなります。
